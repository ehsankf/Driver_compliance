'==========================================================================
' VBS-Script for PTV Vissim
' Copyright (c) Sven Beller, PTV Planung Transport Verkehr GmbH.
' All rights reserved.
' -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -
...
const Path_of_COM_Basic_Commands_network = "C:\Users\Public\Documents\PTV Vision\PTV Vissim 2022\Examples Training\COM\Basic Commands\"
...
Sub LoadLayout()
	Dim filename
	filename = "COM Basic Commands.layx"
...
Sub SingleAttributes()
	...
	MsgBox "Name of Link " & linkNo &" is '" & linkName & "'."

	' Set Link Name:
	linkNameNew = "New Link Name"
	Vissim.Net.Links.ItemByKey(linkNo).AttValue("Name") = linkNameNew
	MsgBox "Name of Link " & linkNo &" is now set to '" & linkNameNew & "'." & vbcrlf & vbcrlf _
	...
	MsgBox "The program number of signal controller " & controllerNo &" is now set to " & programNoNew & "."  & vbcrlf & vbcrlf _
	...
	Dim msgString
  msgString = "The relative flow of Route " & rouDecNo & "-" & routeNo & " is now set to " & relativeFlowNew & "."
  ...
  MsgBox msgString & vbcrlf & vbcrlf +  "About to change the volume of vehicle input no. " & vehInputNo & "..."
  
	volumeNew = 600  ' in vehicles per hour
	Vissim.Net.VehicleInputs.ItemByKey(vehInputNo).AttValue("Volume(1)") = volumeNew
	MsgBox "The volume of the first time interval of vehicle input " & vehInputNo & " is now set to " & volumeNew & "."
  ...
  '	             "AttValue failed: Object 2: Attribute Volume (300) is no subject to changes.")
	...
	MsgBox "The volume of the 2nd time interval of vehicle input " & vehInputNo & " is now set to " & volumeNew & "." & vbcrlf & vbcrlf _
	...
	MsgBox "Vehicle composition " & vehCompositionNo & " is now changed."
...
Sub MultiAttributes()
	...
	MsgBox "All links now have a cost of " & cost
	...
	MsgBox "To the cost of all links an additional value of " & cost & " was added."
...
'              (event-based) scripting, but only from outside Vissim.
  ...
  ' Set the simulation end
	...
	MsgBox "The simulation will now run until the 'Pause at' time at simulation second " & simBreakAt & ". From there you can either continue the simulation (F5) or stop it (Escape).", vbInformation+vbOKOnly ,"Run Simulation" 
...
'              (event-based) scripting, but only from outside Vissim.
  ...
  ' Set simulation 'Pause at'
	Dim simBreakAt
	simBreakAt = 28    ' [Simulation second]
	Vissim.Simulation.AttValue("SimBreakAt") = simBreakAt

 	MsgBox "The simulation will now run until simulation second " & simBreakAt & ". Then various attributes are accessed while the simulation is active. Afterwards the simulation will run until the end of second " & endOfSimulation & "." , vbInformation+vbOKOnly ,"Access Data During Simulation" 
  ...
  signalGroupState = signalGroup.AttValue("SigState")    ' possible return values e.g.: 'GREEN', 'RED', 'AMBER', 'REDAMBER'
	MsgBox "Current state of signal group " & signalGroupNo & " is " + signalGroupState & vbcrlf _
  ...
  '      (every simulation second by default). Hence when changing the state of a signal group through COM, the visible
	...
	MsgBox "The control of signal controller " & controllerNo & " will now " & vbcrlf _
       + "be handed back from COM to the signal controller - " & vbcrlf _
       + "hence the state of signal group " & signalGroupNo & " will change again."
	...
	' --------------------------------------------------------------------------------------
	' Fetch information about the vehicles in the network (in the current simulation second)
	' --------------------------------------------------------------------------------------
	...
	Next
	Dim msgText
  msgText = "Number of vehicles in network: " & Vissim.Net.Vehicles.Count & vbcrlf & vbcrlf _
          + "Information of last vehicle:" & vbcrlf _
          + "vehicle number = " & vehNo & vbCrlf _
          + "Type = " & vehType & vbCrlf _
          + "Speed = " & FormatNumber(vehSpeed,1) & vbCrlf _
          + "Pos = " & FormatNumber(vehPos,2) & vbCrlf _
          + "Link & lane = " & vehLinkLane
  ...
  MsgBox msgText,, "GetMultipleAttributes"
	...
	Dim iteratorFilteredVehicles
	Set iteratorFilteredVehicles = Vissim.Net.Vehicles.GetFilteredSet("[SPEED]<25").Iterator
	While iteratorFilteredVehicles.Valid
			Set vehicle = iteratorFilteredVehicles.Item
			vehNo = vehicle.AttValue("No")
			vehType = vehicle.AttValue("VehType")
			vehSpeed = vehicle.AttValue("Speed")
			vehPos = vehicle.AttValue("Pos")
			vehLinkLane = vehicle.AttValue("Lane")
			iteratorFilteredVehicles.Next
	Wend
	msgText = "Number of vehicles in filtered set: " & Vissim.Net.Vehicles.GetFilteredSet("[SPEED]<25").Count & vbcrlf & vbcrlf _
          + "Information of last vehicle:" & vbcrlf _
          + "vehicle number = " & vehNo & vbCrlf _
          + "Type = " & vehType & vbCrlf _
          + "Speed = " & FormatNumber(vehSpeed,1) & vbCrlf _
  ...
  MsgBox msgText,, "GetFilteredSet"
	...
	Dim desSpeedNew
	desSpeedNew = 30
	MsgBox "Desired speed of vehicle " & vehNo & " is currently " & FormatNumber(vehicle.AttValue("DesSpeed"),1) & "." & vbCrlf _
          & "It will now be set to " & desSpeedNew & "."
	vehicle.AttValue("DesSpeed") = desSpeedNew
	' Run a single simulation step to see the change
	Vissim.Simulation.RunSingleStep 

  MsgBox "Desired speed of vehicle " & vehNo & " is now " & FormatNumber(vehicle.AttValue("DesSpeed"),1) & "."
	...
	laneNo = 1
	linkCoordinate = 25
	MsgBox "Vehicle " & vehNo & " is currently travelling on link " & vehicle.AttValue("Lane") & "." & vbCrlf _
	      & "It will now be moved to link " & linkNo & ", coordinate " & FormatNumber(linkCoordinate,2) & "."
	...
	vehNo = vehicle.AttValue("No")
	MsgBox "Vehicle " & vehNo & " will be removed from the network..."
	...
	linkNo = 13
	laneNo = 1
	linkCoordinate = 30 ' unit according to the user setting in Vissim [m or ft]
	desSpeedNew = 53 ' unit according to the user setting in Vissim [km/h or mph]
	Dim interaction
	interaction = True 	' optional parameter

	MsgBox "A new vehicle will be added on link " & linkNo & ", coordinate " & FormatNumber(linkCoordinate,2) & " with desired speed of " & desSpeedNew & "."
...
Sub GetSimulationRunResults()
	...
	For simRun = 0 To UBound(allSimulationRuns)
			For attIndex = 0 To UBound(attributes)
					resultString = resultString & allSimulationRuns(simRun, attIndex) & ", "
			Next
			resultString = resultString & vbcrlf
	Next
	
	if UBound(allSimulationRuns) < 2 then 
		MsgBox "It seems that not enough simulation runs exist. Please ensure that the simulation was run 3 times with different seeds. You can do so by menu 'Actions - Run script file...' and choosing 'Run 3 simulations.vbs'", vbOKOnly+vbInformation, "Not enough simulation runs"
	else
...
Sub GetTravelTimeResults()
	...
	Dim msgText
  msgText = "Vehicle travel time measurement no. " & vehTravTimeMeasurementNo & ":" & vbcrlf _
        ...
        + "� of all vehicle classes" & vbcrlf _
				...
				+ "= " & FormatNumber(travTime,1) & " s for " & FormatNumber(vehCount,1) & " vehicles."
...
Sub GetDataCollectionResults()	
  ...
  Dim msgText
  msgText = "Vehicle data of data collection measurement no. " & dataCollMeasurementNo & vbcrlf _
				& "� of all vehicle classes" & vbcrlf _
				& "� of the 1st time interval" & vbcrlf _
				& "� averaged over all simulation runs:" & vbcrlf & vbcrlf _ 
				& "Vehicle count = " & vehCount & vbcrlf _ 
				& "Speed = " & FormatNumber(Speed,1) & vbcrlf _ 
				& "Acceleration = " & FormatNumber(Acceleration,1) & vbcrlf _ 
				& "Vehicle length = " & FormatNumber(Length,1)
...
Sub GetQueueLengthResults()
  ...
  Dim msgText
  msgText = "Queue counter no. " & queueCounterNo & ":" & vbcrlf _
				& "Average queue length of all time intervals" & vbcrlf _
				& "and over all simulation runs = " & FormatNumber(queueLenAvg,1)
  MsgBox msgText,,"Queue Length"
        
End Sub

'==========================================================================
Sub GetLinkEvaluationSegmentResults()
	...
	' Syntax to get the data:
	' Vissim.Net.Links.ItemByKey(1).AttValue("Concatenate:LinkEvalSegs\Volume(subAttribute1, subAttribute2, subAttribute3)")
	'
	' 	subAttribute1: simulation run (same as described for vehicle travel time measurements)
	' 	subAttribute2: time interval (of the link segment evaluation)
	' 	subAttribute3: vehicle type

	' ............................................................................
	' Example 1:	Get the average data over all simulationruns (subAttribute1 = Avg)
	'							and over all time intervals (subAttribute2 = Avg)
	'							and for all vehicle typess (subAttribute3 = All)
	' ............................................................................
	Dim linkNo, volume
	linkNo = 1
	volume = Vissim.Net.Links.ItemByKey(linkNo).AttValue("Concatenate:LinkEvalSegs\Volume(Avg, Avg, All)")
	
  Dim msgText
  msgText = "Link no. " & linkno & ":" & vbcrlf _
				& "Average queue length of all simulation runs" & vbcrlf _
				& "and over all time intervals" & vbcrlf _
				& "and for all vehicle types = " & volume
  MsgBox msgText,,"Volume"
...
Sub SaveNetworkAndLayout()
  ...
  filename = "COM Basic Commands VBS - Saved"
	...
	Vissim.SaveNetAs (Path_of_COM_Basic_Commands_network + filename + ".inpx")
	Vissim.SaveLayout (Path_of_COM_Basic_Commands_network + filename + ".layx")
  MsgBox "The network and layout files were saved to "+ vbcrlf & vbcrlf _
         + Path_of_COM_Basic_Commands_network & vbcrlf & vbcrlf _
