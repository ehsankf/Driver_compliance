//= == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == =
// C++ - Script for PTV Vissim
// Copyright (c) PTV Planung Transport Verkehr GmbH.
// All rights reserved.
// -----------------
...
#import "C:\\Program Files\\PTV Vision\\PTV Vissim 2022\\Exe\\Vissim220.exe" rename_namespace ("VISSIMLIB")
...
int main(int argc, char* argv[])
{
	...
	CoInitialize(NULL);
	{
		...
		// Vissim.CreateInstance('Vissim.Vissim.10'); //Vissim 10
		// Vissim.CreateInstance('Vissim.Vissim.22'); //Vissim 2022

		bstr_t Path_of_COM_Basic_Commands_network = "C:\\Users\\Public\\Documents\\PTV Vision\\PTV Vissim 2022\\Examples Training\\COM\\Basic Commands\\"; // always use \\ at the end
		...
		// Read Link Name :
		int Link_number = 1;
		bstr_t Name_of_Link = (bstr_t)Vissim->Net->Links->GetItemByKey(1)->GetAttValue("Name");
		...
		variant_t Names_of_Links = Vissim->Net->Links->GetMultiAttValues(Attribute);

    // GetMultipleAttributes         Read several attributes of all objects at once:
    CComSafeArray<VARIANT> attributes;
    attributes.Add(_variant_t(L"Name"));
    attributes.Add(_variant_t(L"CostPerKm"));
    _variant_t lAttributes;
    lAttributes.vt = VT_VARIANT | VT_ARRAY;
    lAttributes.parray = *attributes.GetSafeArrayPtr();
    auto link_attributes = Vissim->Net->Links
      ->GetMultipleAttributes(lAttributes);
    assert(link_attributes.vt == (VT_VARIANT | VT_ARRAY));
    auto sa = link_attributes.parray;
    CComSafeArray<VARIANT> retrievedAttributes(sa);
    auto dims = retrievedAttributes.GetDimensions();
    assert(dims == 2);
    auto lb0 = retrievedAttributes.GetLowerBound(0);
    auto ub0 = retrievedAttributes.GetUpperBound(0);
    auto lb1 = retrievedAttributes.GetLowerBound(1);
    auto ub1 = retrievedAttributes.GetUpperBound(1);
    VARIANT v;
    LONG pos[2];
    for (auto idx0 = lb0; idx0 <= ub0; ++idx0) {
      pos[0] = idx0;
      pos[1] = 0;
      hr = retrievedAttributes.MultiDimGetAt(pos, v);
      cout << "Name of Link: " << _bstr_t(_variant_t(v).bstrVal) << endl;
      pos[1] = 1;
      hr = retrievedAttributes.MultiDimGetAt(pos, v);
      cout << "CostPerKm of Link: " << _variant_t(v).dblVal << endl;
    }

    // SetMultipleAttributes         Set several attributes of all objects at once:
    CComSafeArray<VARIANT> newAttributes(retrievedAttributes);
    for (auto idx0 = lb0; idx0 <= ub0; ++idx0) {
      pos[0] = idx0;
      pos[1] = 0;
			newAttributes.MultiDimSetAt(pos, _variant_t("OnlyHalfOfTheTruth"));
      pos[1] = 1;
      newAttributes.MultiDimSetAt(pos, _variant_t(21));
    }
    _variant_t newSaV;
    newSaV.vt = VT_VARIANT | VT_ARRAY;
    newSaV.parray = *newAttributes.GetSafeArrayPtr();
    Vissim->Net->Links
      ->SetMultipleAttributes(lAttributes, newSaV);
    link_attributes = Vissim->Net->Links
      ->GetMultipleAttributes(lAttributes);
		retrievedAttributes = link_attributes.parray;
    for (auto idx0 = lb0; idx0 <= ub0; ++idx0) {
      pos[0] = idx0;
      pos[1] = 0;
      hr = retrievedAttributes.MultiDimGetAt(pos, v);
      cout << "New name of Link: " << _bstr_t(_variant_t(v).bstrVal) << endl;
      pos[1] = 1;
      hr = retrievedAttributes.MultiDimGetAt(pos, v);
      cout << "New CostPerKm of Link: " << _variant_t(v).dblVal << endl;
    }

		// SetAllAttValues           Set one attribute of all objects to the same value:
		Attribute = "Name";
		...
		bstr_t State_of_SH = (bstr_t)Vissim->Net->SignalHeads->GetItemByKey(SH_number)->GetAttValue("SigState"); // possible output e.g. 'GREEN', 'RED', 'AMBER', 'REDAMBER'
		...
		//Method #2: Loop over a set of Vehicles with a filter criterion
		VISSIMLIB::IIteratorPtr Vehicles_Filtered_Iterator = Vissim->Net->Vehicles->GetFilteredSet("[SPEED]<25")->GetIterator();
		while (Vehicles_Filtered_Iterator->GetValid())
		{
			Vehicle = Vehicles_Filtered_Iterator->GetItem();
			veh_number = Vehicle->GetAttValue("No");
			veh_type = Vehicle->GetAttValue("VehType");
			veh_speed = Vehicle->GetAttValue("Speed");
			cout << std::setprecision(4) << veh_number << "  |  " << veh_type << "  |  " << veh_speed << endl;
		...
		// As a workaround their values can be accessed via the relations of links.
    bstr_t volAvg = (bstr_t)Vissim->Net->Links->GetItemByKey(1)->GetAttValue("Concatenate:LinkEvalSegs\\Volume(Avg,Avg,All)");
		cout << "Link evaluation results for link #1: " << volAvg << endl;

		//== == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == ==
		// Saving
		//= == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == == =
		Filename = Path_of_COM_Basic_Commands_network + "COM Basic Commands saved.inpx";
		Vissim->SaveNetAs(Filename, false);
