...
// file installed (currently this is Vissim220.exe) in order to use Vissim COM objects.
...
namespace Example
{
    ...
    class Syntax_Example
    {
        ...
        static void Main()
        {
            ...
            Path_of_COM_Basic_Commands_network = "C:\\Users\\Public\\Documents\\PTV Vision\\PTV Vissim 2022\\Examples Training\\COM\\Basic Commands\\"; // always use \\ at the end !!
            ...
            // Method #6: Iterate over a set of Vehicles with a filter criterion
            IIterator Vehicles_Filtered_Iterator = Vissim.Net.Vehicles.GetFilteredSet("[SPEED]<25").Iterator;
            while (Vehicles_Filtered_Iterator.Valid)
            {
              IVehicle Vehicle = (IVehicle)Vehicles_Filtered_Iterator.Item;
              veh_number = (int)Vehicle.get_AttValue("No");
              veh_type = (string)Vehicle.get_AttValue("VehType");
              veh_speed = (double)Vehicle.get_AttValue("Speed");
              veh_position = (double)Vehicle.get_AttValue("Pos");
            ...
            No_Veh_d = (double)Veh_TT_measurement.get_AttValue("Vehs  (Avg,Avg,All)");
            Console.WriteLine("Average travel time all time intervals of all simulation of all vehicle classes: {0:F} (number of vehicles: {1})", TT, No_Veh_d);
            ...
            No_Veh_i = (int)Veh_TT_measurement.get_AttValue("Vehs  (Current,Max,20)");
            Console.WriteLine("Maximum travel time of all time intervals of the current simulation of vehicle class HGV: {0:F} (number of vehicles: {1})", TT, No_Veh_i);
            ...
            // No_Veh_i = (int)Veh_TT_measurement.get_AttValue("Vehs  (2,1,All)");
            // Console.WriteLine("Travel time of the 1st time interval of the 2nd simulation of all vehicle classes: {0:F} (number of vehicles: {1})", TT, No_Veh_i);
            ...
            //   of the 1st time interval (2. input = 1)
            //   of all vehicle classes (3. input = All)
            No_Veh_i = (int)DC_measurement.get_AttValue("Vehs        (Avg,1,All)"); // number of vehicles
            ...
            Console.WriteLine("#vehicles: {0}; Speed: {1:F}; Acceleration: {2:F}; Length: {3:F}", No_Veh_i, Speed, Acceleration, Length);
            ...
            string volAvg = (string)Vissim.Net.Links.get_ItemByKey(1).AttValue["Concatenate:LinkEvalSegs\\Volume(Avg,Avg,All)"];
            Console.WriteLine("Link evaluation results for link #1: {0}", volAvg);
